...
The first system under investigation is an implementation if Iterated Racing Algorithm\cite{birattari2010f} in IRACE package for off-line hyper-parameters tuning.
...
% TODO: I am considering adding a scheme from their paper for illustration if readers found the text-only description hard to grasp.
...
\subsubsection{SMAC~\cite{hutter2011sequential}}\label{bg: smac}
Sequential Model-based Algorithm Configuration (SMAC) is a system for algorithm parameters optimization developed by AutoML research group (here we review SMACv3). In their research, authors shown that the machine learning mechanisms and regression models in particular could be applied not only for tuning parameters, but also for optimizing any expensive black-box functions in general (we believe that the last holds also for the majority of other parameter tuning systems). 

The routine in Sequential Model-based Optimization is somehow similar to one implemented in IRACE, and could be imagined as iterated application two  steps: (1) fitting models, (2) using them for making choices which Configuration to investigate next and actually evaluation of predicted parameters.
The system development was directed to tackle existing (at that point of time) limitations of all published SMBO approaches namely, expand an applicability not only to numerical, but also categorical parameters and optimize target algorithm not only to single, but to number of problem instances (benchmark set of problem instances).
